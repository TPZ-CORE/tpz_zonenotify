	...
	if hash ~= false then
		...
		if sd then
			...
			if Config.OutOfTownNotification then
				TriggerEvent("tpz_core:sendTopNotification", Locales["OUT_OF_TOWN"], Config.Notification.TimeShowing)
			end
...
function nativeAlertTop() 
    ...
    if zone then
        ...
        TriggerEvent("tpz_core:sendTopNotification", text, zone, Config.Notification.TimeShowing)
    end
end
